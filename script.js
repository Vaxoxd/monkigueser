...
function startRound() {
  ...
  document.getElementById("instruction").innerText = "Kliknij na mapie, aby zaznaczyć swoją lokalizację!";
  resetMarkers();
...
function showNextNotification() {
  ...
  notification.className = `purchase-notification ${notificationData.type}`;
  ...
  const notificationData = {
    ...
    html: `
      ...
      <div class="notification-text">
        ...
        <div class="notification-desc">${itemName}</div>
        <div class="notification-detail">${description}</div>
      </div>
    `
  };
...
function endMultiplayerGame() {
  ...
  if (multiplayerState.myScore > multiplayerState.opponentScore) {
    ...
    totalPoints += 2000; // Bonus za wygraną w PvP
    if (currentUser) {
      updateUserStats(2000, 'win');
    }
    ...
    totalPoints += 500; // Pocieszenie
    if (currentUser) {
      updateUserStats(500, 'loss');
    }
  } else {
    gameResult = "🤝 REMIS!";
    resultClass = "draw";
